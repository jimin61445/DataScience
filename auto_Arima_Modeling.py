...
for line in lines:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            # ARIMA 모델 적용
            model = pm.auto_arima(log_time_series, seasonal=False, stepwise=True, suppress_warnings=True, trace=True)
            model_fit = model

            # 예측
            forecast_log = model_fit.predict(n_periods=len(time_columns))
            forecast = np.expm1(forecast_log)
            forecast = np.clip(forecast, 0, None)  # 음수값을 0으로 클리핑
